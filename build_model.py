...
def inference(images):
	...
	with tf.variable_scope('low_level_conv1') as scope:
		...
		low_level_conv1 = tf.nn.relu(pre_activation, name=scope.name)
	...
	with tf.variable_scope('low_level_conv2') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(low_level_conv1, kernel, [1, 1, 1, 1], padding='VALID')
	...
	with tf.variable_scope('low_level_conv3') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(low_level_conv2, kernel, [1, 2, 2, 1], padding='VALID')
		biases = _variable_on_cpu('biases', [128], tf.constant_initializer(0.0))
		pre_activation = tf.nn.bias_add(conv, biases)
	...
	with tf.variable_scope('low_level_conv4') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(low_level_conv3, kernel, [1, 1, 1, 1], padding='VALID')
	...
	with tf.variable_scope('low_level_conv5') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(low_level_conv4, kernel, [1, 2, 2, 1], padding='VALID')
		biases = _variable_on_cpu('biases', [256], tf.constant_initializer(0.0))
		pre_activation = tf.nn.bias_add(conv, biases)
	...
	with tf.variable_scope('low_level_conv6') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(low_level_conv5, kernel, [1, 1, 1, 1], padding='VALID')
	...
	with tf.variable_scope('mid_level_conv1') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(low_level_conv6, kernel, [1, 1, 1, 1], padding='VALID')
		biases = _variable_on_cpu('biases', [512], tf.constant_initializer(0.0))
		pre_activation = tf.nn.bias_add(conv, biases)
	...
	with tf.variable_scope('mid_level_conv2') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(mid_level_conv1, kernel, [1, 1, 1, 1], padding='VALID')
		biases = _variable_on_cpu('biases', [256], tf.constant_initializer(0.0))
		pre_activation = tf.nn.bias_add(conv, biases)
	...
	with tf.variable_scope('global_level_conv1') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(low_level_conv6, kernel, [1, 2, 2, 1], padding='VALID')
		biases = _variable_on_cpu('biases', [512], tf.constant_initializer(0.0))
		pre_activation = tf.nn.bias_add(conv, biases)
	...
	with tf.variable_scope('global_level_conv2') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(global_level_conv1, kernel, [1, 1, 1, 1], padding='VALID')
		biases = _variable_on_cpu('biases', [512], tf.constant_initializer(0.0))
		pre_activation = tf.nn.bias_add(conv, biases)
	...
	with tf.variable_scope('global_level_conv3') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(global_level_conv2, kernel, [1, 2, 2, 1], padding='VALID')
		biases = _variable_on_cpu('biases', [512], tf.constant_initializer(0.0))
		pre_activation = tf.nn.bias_add(conv, biases)
	...
	with tf.variable_scope('global_level_conv4') as scope:
		kernel = _variable_with_weight_decay('weights',
		                                     ...
		                                     stddev=5e-2,
		                                     wd=0.0)
		conv = tf.nn.conv2d(global_level_conv3, kernel, [1, 1, 1, 1], padding='VALID')
		biases = _variable_on_cpu('biases', [512], tf.constant_initializer(0.0))
		pre_activation = tf.nn.bias_add(conv, biases)
